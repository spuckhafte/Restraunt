...
export default function ClerkAnimation({ onComplete }: { onComplete?: () => void }) {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= FRAME_COUNT; i++) {
      const img = new Image();
      img.src = `/clerkAnimated/ezgif-frame-${i.toString().padStart(3, "0")}.jpg`;
    ...
    <div className="w-full h-screen bg-black flex items-center justify-center relative">
      {!isLoaded ? (
        <div className="z-10 bg-black/80 backdrop-blur-sm p-6 rounded-2xl border border-white/5 flex flex-col items-center">
          <div className="w-8 h-8 border-t-2 border-fuchsia-400 border-r-2 border-r-cyan-400 rounded-full animate-spin mb-4"></div>
          <p className="bg-gradient-to-r from-fuchsia-400 to-cyan-400 bg-clip-text text-transparent font-light tracking-widest uppercase text-xs animate-pulse">
            Approaching Clerk Station...
          </p>
        </div>
      ) : useFallback ? (
        <video
          src="/clerkAnimated.mp4"
          autoPlay
          muted
          playsInline

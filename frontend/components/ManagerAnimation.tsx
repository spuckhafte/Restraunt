...
export default function ManagerAnimation({ onComplete }: { onComplete?: () => void }) {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= FRAME_COUNT; i++) {
      const img = new Image();
      img.src = `/managerAnimated/ezgif-frame-${i.toString().padStart(3, "0")}.jpg`;
    ...
    <div className="w-full h-screen bg-black flex items-center justify-center relative">
      {!isLoaded ? (
        <div className="z-10 bg-black/80 backdrop-blur-sm p-6 rounded-2xl border border-white/5 flex flex-col items-center">
          ...
          <p className="text-cyan-300 font-light tracking-widest uppercase text-xs animate-pulse">
            Accessing Managerial Floor...
          </p>
        </div>
      ) : useFallback ? (
        <video
          src="/managerAnimated.mp4"
          autoPlay
          muted
          playsInline
